...
//--- clean up arrays
   ...
   for(int i=rates_total-3; i >= start && !IsStopped(); i--)
     {
      //--- Upper Fractal
      if(high[i]>high[i+1] && high[i]>high[i+2] && high[i]>=high[i-1] && high[i]>=high[i-2])
        {
         ...
         string status = "";
         double ch = high[i];
         for(int c=0; c <= rates_total-3 ; c++)
           {
            if(ExtUpperBuffer[c] != EMPTY_VALUE)
              {
               double ph = ExtUpperBuffer[c];
               status = (ch > ph) ? "HH" : "LH";
               plotTimes(0, "amFract"+(string)i, time[i],"ph= "+ ph, high[i], clrRed);
               break;
              }
           }
        }
      ...
      if(low[i]<low[i+1] && low[i]<low[i+2] && low[i]<=low[i-1] && low[i]<=low[i-2])
        {
         ...
         string status = "";
         double cl = low[i];
         for(int c=i+1; c <= rates_total-3; c++)
           {
            if(ExtLowerBuffer[c] != EMPTY_VALUE && ExtLowerBuffer[c] !=cl)
              {
               ...
               if(cl > pl)
                  ...
                  status = "LL";
               plotTimes(0, "amFract"+(string)i, time[i],"pl= "  +pl, low[i], clrGreen);
               break;
              }
           }
        }
